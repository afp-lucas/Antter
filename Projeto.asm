...
;	- Nao apaga o cenario
;	- Le teclado para movimentar a Formiga
;	- Declara Tabela de nr. Randoicos
;	- Le Tabela de nr. Randomico para movimentar o Alien
;	- Se Tiro ou Alien passar por cima, nao apaga a Formiga
...
;		if (mod(c/10)==0
;			{	
;				RecalculaFormiga (posFormiga = Teclado...)
;				If (posFormiga != posAntFormiga)
;				{ 	ApagaFormiga: Print Tela(posAntFormiga + posAntFormiga/40) , posAntFormiga
;				 	DesenhaFormiga  (posAntFormiga = posFormiga)
;				}
;			}
;		if (mod(c/5)==0
;			{	
...
posFormiga: var #1			; Contem a posicao atual da Formiga
posAntFormiga: var #1		; Contem a posicao anterior da Formiga
...
main:
	...
	Loadn R0, #0			
	store posFormiga, R0		; Zera Posicao Atual da Formiga
	store posAntFormiga, R0	; Zera Posicao Anterior da Formiga
	...
	Loop:
		...
		ceq MoveFormiga	; Chama Rotina de movimentacao da Formiga
...
MoveFormiga:
	...
	call MoveFormiga_RecalculaPos		; Recalcula Posicao da Formiga

; So' Apaga e Redezenha se (pos != posAnt)
;	If (posFormiga != posAntFormiga)	{	
	load r0, posFormiga
	load r1, posAntFormiga
	cmp r0, r1
	jeq MoveFormiga_Skip
		call MoveFormiga_Apaga
		call MoveFormiga_Desenha		;}
  MoveFormiga_Skip:
...
MoveFormiga_Apaga:		; Apaga a Formiga preservando o Cenario!
	push R0
	push R1
	push R2
	push R3
	...
	load R0, posAntFormiga	; R0 = posAnt
...
MoveFormiga_RecalculaPos:		; Recalcula posicao da Formiga em funcao das Teclas pressionadas
	push R0
	push R1
	push R2
	push R3
	...
	inchar R1				; Le Teclado para controlar a Formiga
	loadn R2, #'a'
	cmp R1, R2
  ...
  MoveFormiga_RecalculaPos_Fim:	; Se nao for nenhuma tecla valida, vai embora
	store posFormiga, R0
	pop R3
	pop R2
	pop R1
  ...
  MoveFormiga_RecalculaPos_A:	; Move Formiga para Esquerda
	loadn R1, #40
	loadn R2, #0
	mod R1, R0, R1		; Testa condicoes de Contorno 
	cmp R1, R2
	jeq MoveFormiga_RecalculaPos_Fim
	dec R0	; pos = pos -1
	jmp MoveFormiga_RecalculaPos_Fim
		
  MoveFormiga_RecalculaPos_D:	; Move Formiga para Direita	
	loadn R1, #40
	loadn R2, #39
	mod R1, R0, R1		; Testa condicoes de Contorno 
	cmp R1, R2
  ...
  MoveFormiga_RecalculaPos_W:	; Move Formiga para Cima
	loadn R1, #40
	cmp R0, R1		; Testa condicoes de Contorno 
	jle MoveFormiga_RecalculaPos_Fim
	sub R0, R0, R1	; pos = pos - 40
	jmp MoveFormiga_RecalculaPos_Fim

  MoveFormiga_RecalculaPos_S:	; Move Formiga para Baixo
	loadn R1, #1159
	cmp R0, R1		; Testa condicoes de Contorno 
	jgr MoveFormiga_RecalculaPos_Fim
	loadn R1, #40
...
;----------------------------------
MoveFormiga_Desenha:	; Desenha caractere da Formiga
	push R0
	push R1
	push R5
	push R6
	...
	add R6, R0, R5
	outchar R7, R6
	store posAntFormiga, R0	; Atualiza Posicao Anterior da Formiga = Posicao Atual
...
MoveAlien_Apaga:
	...
	load R1, posAntFormiga		; R1 = posAnt
	cmp r0, r1
	jne MoveAlien_Apaga_Skip
		loadn r5, #'X'		; Se o Tiro passa sobre a Formiga, apaga com um X, senao apaga com o cenario 
